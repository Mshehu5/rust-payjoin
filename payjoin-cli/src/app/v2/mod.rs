...
use payjoin::relay_selection::RequestKind;
use payjoin::send::v2::{
...
use crate::app::v2::ohttp::{classify_reqwest_error, MailroomManager, RelayAttemptError};
...
impl AppTrait for App {
    ...
    async fn receive_payjoin(&self, amount: Amount) -> Result<()> {
        ...
        let network = network::MailroomUrlResolver::new(v2_config);
        let (network_selection, ohttp_keys) =
            self.mailroom_manager.bootstrap_receiver(&network).await?;
            ...
            match replay_receiver_event_log(&recv_persister) {
                Ok((receiver_state, history)) => {
                    ...
                    let relay_selector = match relay_selection::relay_selector_from_endpoint(
                        ...
                        Ok(relay_selector) => relay_selector,
                        Err(error) => {
                            tracing::error!(
...
impl App {
        ...
        loop {
            ...
            match sender.process_response(&response.bytes().await?, ctx).save(persister) {
                Ok(sender) => {
                    ...
                    return self.get_proposed_payjoin_psbt(sender, persister, relay_selector).await;
                }
        ...
        loop {
            let (response, ctx) = self
                .send_ohttp_request_with_relay_selector(
                    relay_selector,
                    RequestKind::Poll,
                    |relay| sender.create_poll_request(relay.as_str()).map_err(Into::into),
                )
                .await?;
        ...
        loop {
            let (err_response, err_ctx) = self
                .send_ohttp_request_with_relay_selector(
                    ...
                    |relay| {
                        session
                            .create_error_request(relay.as_str())
    ...
    async fn post_request(
        ...
        if let Some(domain) = relay.url.domain() {
