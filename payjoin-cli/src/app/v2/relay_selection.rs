...
fn relay_candidates(
    ...
    let relays = v2
        ...
        .iter()
        .map(|url| {
            resolve_server(network, url).map(|target| RelayCandidate::individual(target.resolved))
        })
...
#[cfg(feature = "asmap")]
// CLI/app code: DNS plus ASMap lookup. The resulting ASN is passed to the
// library selector as an opaque privacy-bucket identifier.
